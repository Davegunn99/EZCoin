...
*Setup instructions for a Gitian build of EZCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the EZCoin
...
- [Building EZCoin Core](#building-ezcoin-core)
...
Clone the git repositories for ezcoin and Gitian.
...
in the ezcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building EZCoin Core
----------------

To build EZCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the ezcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ezcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/Davegunn99/EZCoin
...
For example:
```bash
URL=https://github.com/thrasher-/ezcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit ezcoin=${COMMIT} --url ezcoin=${URL} ../ezcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ezcoin=${COMMIT} --url ezcoin=${URL} ../ezcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ezcoin=${COMMIT} --url ezcoin=${URL} ../ezcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the ezcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../ezcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/ezcoin
SIGPATH=/some/root/path/ezcoin-detached-sigs

./bin/gbuild --url ezcoin=${BTCPATH},signature=${SIGPATH} ../ezcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ezcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ezcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/ezcoin-osx-build.assert
```
...
[ezcoin-project/gitian.sigs.ezc](https://github.com/ezcoin-project/gitian.sigs.ezc/) repository, or if that's not possible create a pull
