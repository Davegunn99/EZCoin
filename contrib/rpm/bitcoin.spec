...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 44807
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 44804
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 44807
	%{_sbindir}/semanage port -d -p tcp 44804
